...
.. warning::
   ...
   fMRIPost-Phase currently only generates denoised data in MNI152NLin6Asym space.

Defining standard and nonstandard spaces where data will be resampled
=====================================================================
The command line interface of *fMRIPost-Phase* allows resampling the preprocessed data
...
.. important::
   *fMRIPost-Phase* will reduce the amount of output spaces to just spaces listed in ``--output-spaces``,
...
that allows end-user applications such as *fMRIPost-Phase* to flexibly query and pull
template and atlas information.
In other words, *TemplateFlow* enables *fMRIPost-Phase* to access a wide range
of templates (and also custom templates, see below).
Therefore, *TemplateFlow* is central to define *fMRIPost-Phase*'s interface regarding
...
Standard spaces
"""""""""""""""
When using *fMRIPost-Phase* in a workflow that will investigate effects that span across
...
For instance, to instruct *fMRIPost-Phase* to use the MNI template brain distributed with
FSL as coordinate reference the option will read as follows: ``--output-spaces MNI152NLin6Asym``.
By default, *fMRIPost-Phase* uses ``MNI152NLin6Asym`` as spatial-standardization reference.
...
Therefore, *fMRIPost-Phase* will run nonlinear registration processes against the template
T1w image corresponding to all the standard spaces supplied with the argument
``--output-spaces``.
By default, *fMRIPost-Phase* will resample the preprocessed data on those spaces (labeling the
...
""""""""""""""""""""""
To make your custom templates visible by *fMRIPost-Phase*, and usable via
...
* ``fsnative``: similarly to the ``anat`` space for volumetric references,
  including the ``fsnative`` space will instruct *fMRIPost-Phase* to sample the
...
In other words, running *fMRIPost-Phase* with ``--output-spaces MNI152NLin6Asym:res-2
