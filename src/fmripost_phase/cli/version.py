...
def check_latest():
    ...
    cachefile = Path.home() / '.cache' / 'fmripost_phase' / 'latest'
    try:
    ...
    if latest is None or outdated is True:
        ...
        with suppress(Exception):
            response = requests.get(url='https://pypi.org/pypi/fmripost_phase/json', timeout=1.0)
...
def is_flagged():
    ...
    # https://raw.githubusercontent.com/nipreps/fmripost_phase/main/.versions.json
    flagged = ()
    response = None
    with suppress(Exception):
