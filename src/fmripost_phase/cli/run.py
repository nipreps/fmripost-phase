...
def main():
    ...
    from fmripost_phase.cli.parser import parse_args
    from fmripost_phase.cli.workflow import build_workflow
    from fmripost_phase.utils.bids import write_bidsignore, write_derivative_description
    ...
    if 'pdb' in config.execution.debug:
        from fmripost_phase.utils.debug import setup_exceptionhook
    ...
    if not config.execution.notrack and not config.execution.debug:
        ...
        from fmripost_phase.utils.telemetry import sentry_setup, setup_migas
    ...
    fmripost_phase_wf = retval.get('workflow', None)

    # CRITICAL Load the config from the file. This is necessary because the ``build_workflow``
    # function executed constrained in a process may change the config (and thus the global
    # state of fMRIPost-Phase).
    config.load(config_file)

    if config.execution.reports_only:
        sys.exit(int(EXITCODE > 0))

    if fmripost_phase_wf and config.execution.write_graph:
        fmripost_phase_wf.write_graph(graph2use='colored', format='svg', simple_form=True)

    EXITCODE = EXITCODE or (fmripost_phase_wf is None) * EX_SOFTWARE
    if EXITCODE != 0:
        sys.exit(EXITCODE)

    # Generate boilerplate
    with Manager() as mgr:
        from fmripost_phase.cli.workflow import build_boilerplate

        p = Process(target=build_boilerplate, args=(str(config_file), fmripost_phase_wf))
        p.start()
        p.join()
    ...
    if sentry_sdk is not None:
        ...
        sentry_sdk.add_breadcrumb(message='fMRIPost-Phase started', level='info')
        sentry_sdk.capture_message('fMRIPost-Phase started', level='info')

    config.loggers.workflow.log(
        15,
        '\n'.join(['fMRIPost-Phase config:'] + [f'\t\t{s}' for s in config.dumps().splitlines()]),
    )
    config.loggers.workflow.log(25, 'fMRIPost-Phase started!')
    errno = 1  # Default is error exit unless otherwise set
    try:
        fmripost_phase_wf.run(**config.nipype.get_plugin())
    except Exception as e:
        if not config.execution.notrack:
            from fmripost_phase.utils.telemetry import process_crashfile
        ...
        config.loggers.workflow.critical('fMRIPost-Phase failed: %s', e)
        raise

    else:
        config.loggers.workflow.log(25, 'fMRIPost-Phase finished successfully!')
        if sentry_sdk is not None:
            success_message = 'fMRIPost-Phase finished without errors'
        ...
        if boiler_file.exists():
            ...
            config.loggers.workflow.log(
                25,
                'Works derived from this fMRIPost-Phase execution should include the '
        ...
        from fmripost_phase.reports.core import generate_reports
...
def migas_exit() -> None:
    ...
    from fmripost_phase.utils.telemetry import send_breadcrumb
...
if __name__ == '__main__':
    raise RuntimeError(
        'fmripost_phase/cli/run.py should not be run directly;\n'
        'Please `pip install` fmripost_phase and use the `fmripost_phase` command'
    )
