...
``multiprocessing.Process`` that allows fmripost_phase to enforce
...
def build_workflow(config_file, retval):
    ...
    banner = [f'Running fMRIPost-Phase version {version}']
    notice_path = Path(pkgrf('fmripost_phase', 'data/NOTICE'))
    if notice_path.exists():
        banner[0] += '\n'
        banner += [f'License NOTICE {"#" * 50}']
        banner += [f'fMRIPost-Phase {version}']
    ...
    # Build main workflow
    init_msg = [
        "Building fMRIPost-Phase's workflow:",
    ...
    retval['workflow'] = init_fmripost_phase_wf()

    # Check workflow for missing commands
    missing = check_deps(retval['workflow'])
    if missing:
        build_log.critical(
            'Cannot run fMRIPost-Phase. Missing dependencies:%s',
    ...
    build_log.info(
        'fMRIPost-Phase workflow graph with %d nodes built successfully.',
...
def build_boilerplate(config_file, workflow):
    ...
    if not config.execution.md_only_boilerplate and citation_files['md'].exists():
        ...
        bib_text = Path(pkgrf('fmripost_phase', 'data/boilerplate.bib')).read_text()
        citation_files['bib'].write_text(
            bib_text.replace(
                'fMRIPost-Phase <version>',
                f'fMRIPost-Phase {config.environment.version}',
            )
        )
        ...
        cmd = [
            ...
            '--citeproc',
            '--metadata',
            'pagetitle="fMRIPost-Phase citation boilerplate"',
