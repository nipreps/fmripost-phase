...
def _build_parser(**kwargs):
    ...
    from fmripost_phase.cli.version import check_latest, is_flagged
    ...
    verstr = f'fMRIPost-phase v{config.environment.version}'
    currentv = Version(config.environment.version)
    is_release = not any((currentv.is_devrelease, currentv.is_prerelease, currentv.is_postrelease))

    parser = ArgumentParser(
        description=(
            f'fMRIPost-phase: fMRI POSTprocessing of Phase workflow v{config.environment.version}'
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'fMRIPost-phase (see BIDS-Apps specification).'
        ),
    )
    ...
    g_perfm.add_argument(
        ...
        type=_to_gb,
        metavar='MEMORY_MB',
        help='Upper bound memory limit for fMRIPost-phase processes',
    )
    g_perfm.add_argument(
        '--low-mem',
        action='store_true',
...
https://fmriprep.readthedocs.io/en/%s/spaces.html"""
    ...
    g_other.add_argument(
        ...
        'recommended when running concurrent processes of fMRIPost-phase.',
    )
    g_other.add_argument(
        '--resource-monitor',
        action='store_true',
    ...
    if latest is not None and currentv < latest:
        print(
            f"""\
You are using fMRIPost-phase-{currentv},
and a newer version of fMRIPost-phase is available: {latest}.
...
https://fmriprep.readthedocs.io/en/latest/faq.html#upgrading""",
    ...
    if _blist[0]:
        ...
        print(
            f"""\
WARNING: Version {config.environment.version} of fMRIPost-phase (current) has been FLAGGED
(reason: {_reason}).
...
def parse_args(args=None, namespace=None):
    ...
    if config.execution.fmripost_phase_dir is None:
        config.execution.fmripost_phase_dir = output_dir

    # Wipe out existing work_dir
    if opts.clean_workdir and work_dir.exists():
        from niworkflows.utils.misc import clean_directory

        build_log.info(f'Clearing previous fMRIPost-phase working directory: {work_dir}')
    ...
    if output_dir == bids_dir:
        parser.error(
            ...
            f"(suggestion: {bids_dir / 'derivatives' / 'fmripost_phase-' + version.split('+')[0]}."
        )
