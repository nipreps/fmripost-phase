...
# NOTE: Modified for fmripost_phase's purposes
fmripost_phase_spec = loads(load_data('io_spec.json').read_text())
bids_config = Config.load('bids')
deriv_config = Config.load('derivatives')

fmripost_phase_entities = {v['name']: v['pattern'] for v in fmripost_phase_spec['entities']}
merged_entities = {**bids_config.entities, **deriv_config.entities}
merged_entities = {k: v.pattern for k, v in merged_entities.items()}
merged_entities = {**merged_entities, **fmripost_phase_entities}
...
class DerivativesDataSink(BaseDerivativesDataSink):
    ...
    using fmripost_phase's configuration files.
    """
    ...
    _file_patterns = fmripost_phase_spec['patterns']
