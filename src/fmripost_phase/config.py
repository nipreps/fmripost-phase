...
#
r"""
A Python module to maintain unique, run-wide *fMRIPost-Phase* settings.

This module implements the memory structures to keep a consistent, singleton config.
Settings are passed across processes via filesystem, and a copy of the settings for
each run and subject is left under
``<fmriprep_dir>/sub-<participant_id>/log/<run_unique_id>/fmripost_phase.toml``.
Settings are stored using :abbr:`ToML (Tom's Markup Language)`.
The module has a :py:func:`~fmripost_phase.config.to_filename` function to allow writing out
the settings to hard disk in *ToML* format, which looks like:

.. literalinclude:: ../fmripost_phase/data/tests/config.toml
   :language: toml
   :name: fmripost_phase.toml
   :caption: **Example file representation of fMRIPost-Phase settings**.

This config file is used to pass the settings across processes,
using the :py:func:`~fmripost_phase.config.load` function.
...
.. code-block:: Python
    ...
    config_file = config.execution.work_dir / '.fmripost_phase.toml'
...
CONFIG_FILENAME = 'fmripost_phase.toml'
...
if not any(
    ...
    # https://github.com/nipreps/fmripost_phase/pull/2080#discussion_r409118765
    from fmriprep._warnings import logging
else:
    import logging
...
if os.getenv('IS_DOCKER_8395080871'):
    ...
    if _cgroup.exists() and 'docker' in _cgroup.read_text():
        ...
        _exec_env = 'fmripost_phase-docker' if _docker_ver else 'docker'
    del _cgroup
...
class environment(_Config):
    ...
    execution environment, nipype and *fMRIPost-Phase* versions, etc.).
...
class execution(_Config):
    ...
    notrack = False
    """Do not collect telemetry information for *fMRIPost-Phase*."""
    track_carbon = False
...
class workflow(_Config):
    ...
    err_on_warn = False
    """Cast Phase warnings to errors."""
    gift_dimensionality = None
    """Dimensionality of the GIFT ICA."""
    jolt = None
    ...
    ignore = None
    """Ignore particular steps for *fMRIPost-Phase*."""
    cifti_output = None
...
def load(filename, skip=None, init=True):
    ...
    filename : :py:class:`os.PathLike`
        TOML file containing fMRIPost-Phase configuration.
    skip : dict or None
